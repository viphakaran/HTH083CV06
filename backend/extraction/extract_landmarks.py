...
# Setup HandLandmarker compatibility wrapper to match exact prompt API:
...
# Exact configuration as specified in 5.1:
...
def process_clip(gloss, video_path):
    ...
    num_frames = len(position_features_list)
    if num_frames == 0:
        return False, "0 frames extracted"
        
    # Step 5.5: Check consecutive empty frames
    max_consecutive_empty = 0
    current_consecutive = 0
    for is_empty in empty_frames_mask:
        if is_empty:
    ...
    pos_array = np.array(position_features_list, dtype=np.float32) # shape (num_frames, 128)
    
    # Linear interpolation if any empty frames (< 3 consecutive)
    valid_indices = [i for i, is_empty in enumerate(empty_frames_mask) if not is_empty]
...
for idx, (gloss, video_path) in enumerate(tqdm(video_tasks, desc="Extracting landmarks")):
    ...
    if ok:
        ...
        failed_clips += 1
        # Update manifest if skipped due to consecutive empty frames or error
