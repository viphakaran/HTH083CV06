...
TEMPLATES_PATH = Path(__file__).resolve().parent / "phrase_templates.json"
...
class PhraseBuilder:
    ...
    def build_phrase(self, tokens: List[str]) -> Dict[str, Any]:
        ...
        for key, tmpl in self.templates.items():
            tmpl_set = set(t.lower().strip().replace(" ", "") for t in tmpl["tokens"])
...
if __name__ == "__main__":
    pb = PhraseBuilder()
    print("Test build_phrase(['wait', 'time']):", pb.build_phrase(["wait", "time"]))
