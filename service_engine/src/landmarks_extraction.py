...
def draw(image, results):
    ...
    Face mapping has been removed to preserve user privacy and focus exclusively on hand gestures.

    Args:
        image (numpy.ndarray): Input image.
        results: Prediction results containing the landmarks.
    """
    # Face mapping removed - only draw hand gesture landmarks
...
def extract_coordinates(results):
    ...
    Face mapping is excluded; face landmarks are filled with NaN to preserve
    543-element shape compatibility for the classification model. Only hand
    gestures (left and right) and pose landmarks are extracted.

    Args:
        results: Prediction results containing the landmarks.

    Returns:
        numpy.ndarray: Array of extracted coordinates (shape: 543, 3).
    """
    ...
    lh = np.array([[res.x, res.y, res.z] for res in results.left_hand_landmarks.landmark]) if results.left_hand_landmarks else np.full((21, 3), np.nan)
    rh = np.array([[res.x, res.y, res.z] for res in results.right_hand_landmarks.landmark]) if results.right_hand_landmarks else np.full((21, 3), np.nan)
